                                        ...
                                        <div class="col-xl-4 col-sm-6">
                                            <div class="single_ads_card mt-30">
                                                <div class="ads_card_image">
                                                    <img src="{{ asset('storage/advertisement/'.$mobile->image->photo0)}}" alt="ads">
                                                </div>
                                                <div class="ads_card_content">
                                                    <div class="meta d-flex justify-content-between">
                                                        <p>{{$mobile->category->category_name}} </p>
                                                        @livewire('ads.like', ['ad_id' => $mobile->id])
                                                    </div>
                                                    <h4 class="title">
                                                        <a href="{{route('ad.show',[app()->getLocale(), $mobile->slug])}} ">
                                                            {{$mobile->title}}
                                                        </a>
                                                    </h4>
                                                    <p><i class="far fa-map-marker-alt"></i>
                                                        {{$mobile->city->state->country->name}}-
                                                        {{$mobile->city->state->name}}-
                                                        {{$mobile->city->name}}
                                                    </p>
                                                    <div class="ads_price_date d-flex justify-content-between">
                                                        <span class="price">
                                                            {{$mobile->price}} {{ $mobile->currency == 'Dollar' ? '$' : ($mobile->currency == 'Euro' ? '€' : 'FCFA') }}
                                                        </span>
                                                        <span class="date">{{$mobile->created_at->diffForHumans()}} </span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                @forelse ($mobiles as $mobile)
                                    <div class="single_ads_card ads_list d-sm-flex mt-30">
                                        ...
                                        <div class="ads_card_content media-body">
                                            ...
                                            <h4 class="title">
                                                <a href="#">{{$mobile->category->category_name}}</a>
                                            </h4>
                                            <p><i class="far fa-map-marker-alt"></i>
                                                {{$mobile->city->state->country->name}}-
                                                {{$mobile->city->state->name}}-
