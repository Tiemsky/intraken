                                        ...
                                        <div class="col-xl-4 col-sm-6">
                                            <div class="single_ads_card mt-30">
                                                <div class="ads_card_image">
                                                    <img src="{{ asset('storage/advertisement/'.$electronic->image->photo0)}}" alt="ads">
                                                </div>
                                                <div class="ads_card_content">
                                                    <div class="meta d-flex justify-content-between">
                                                        <p>{{$electronic->category->category_name}} </p>
                                                        @livewire('ads.like', ['ad_id' => $electronic->id])
                                                    </div>
                                                    <h4 class="title">
                                                        <a href="{{route('ad.show',[app()->getLocale(), $electronic->slug])}} ">
                                                            {{$electronic->title}}
                                                        </a>
                                                    </h4>
                                                    <p><i class="far fa-map-marker-alt"></i>
                                                        {{$electronic->city->state->country->name}}-
                                                        {{$electronic->city->state->name}}-
                                                        {{$electronic->city->name}}
                                                    </p>
                                                    <div class="ads_price_date d-flex justify-content-between">
                                                        <span class="price">
                                                            {{$electronic->price}} {{ $electronic->currency == 'Dollar' ? '$' : ($electronic->currency == 'Euro' ? '€' : 'FCFA') }}
                                                        </span>
                                                        <span class="date">{{$electronic->created_at->diffForHumans()}} </span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                @forelse ($electronics as $electronic)
                                    <div class="single_ads_card ads_list d-sm-flex mt-30">
                                        ...
                                        <div class="ads_card_content media-body">
                                            ...
                                            <h4 class="title">
                                                <a href="#">{{$electronic->category->category_name}}</a>
                                            </h4>
                                            <p><i class="far fa-map-marker-alt"></i>
                                                {{$electronic->city->state->country->name}}-
                                                {{$electronic->city->state->name}}-
