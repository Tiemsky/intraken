                                        ...
                                        <div class="col-xl-4 col-sm-6">
                                            <div class="single_ads_card mt-30">
                                                <div class="ads_card_image">
                                                    <img src="{{ asset('storage/advertisement/'.$furniture->image->photo0)}}" alt="ads">
                                                </div>
                                                <div class="ads_card_content">
                                                    <div class="meta d-flex justify-content-between">
                                                        <p>{{$furniture->category->category_name}} </p>
                                                        @livewire('ads.like', ['ad_id' => $furniture->id])
                                                    </div>
                                                    <h4 class="title">
                                                        <a href="{{route('ad.show',[app()->getLocale(), $furniture->slug])}} ">
                                                            {{$furniture->title}}
                                                        </a>
                                                    </h4>
                                                    <p><i class="far fa-map-marker-alt"></i>
                                                        {{$furniture->country->name}}-
                                                        {{$furniture->state->name}}-
                                                        {{$furniture->city->name}}
                                                    </p>
                                                    <div class="ads_price_date d-flex justify-content-between">
                                                        <span class="price">
                                                            {{$furniture->price}} {{ $furniture->currency == 'Dollar' ? '$' : ($furniture->currency == 'Euro' ? '€' : 'FCFA') }}
                                                        </span>
                                                        <span class="date">{{$furniture->created_at->diffForHumans()}} </span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                @forelse ($furnitures as $furniture)
                                    <div class="single_ads_card ads_list d-sm-flex mt-30">
                                        ...
                                        <div class="ads_card_content media-body">
                                            ...
                                            <h4 class="title">
                                                <a href="#">{{$furniture->category->category_name}}</a>
                                            </h4>
                                            <p><i class="far fa-map-marker-alt"></i>
                                                {{$furniture->country->name}}-
                                                {{$furniture->state->name}}-
