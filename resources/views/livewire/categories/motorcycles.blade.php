                                        ...
                                        <div class="col-xl-4 col-sm-6">
                                            <div class="single_ads_card mt-30">
                                                <div class="ads_card_image">
                                                    <img src="{{ asset('storage/advertisement/'.$motorcycle->image->photo0)}}" alt="ads">
                                                </div>
                                                <div class="ads_card_content">
                                                    <div class="meta d-flex justify-content-between">
                                                        <p>{{$motorcycle->category->category_name}} </p>
                                                        @livewire('ads.like', ['ad_id' => $motorcycle->id])
                                                    </div>
                                                    <h4 class="title">
                                                        <a href="{{route('ad.show',[app()->getLocale(), $motorcycle->slug])}} ">
                                                            {{$motorcycle->title}}
                                                        </a>
                                                    </h4>
                                                    <p><i class="far fa-map-marker-alt"></i>
                                                        {{$motorcycle->country->name}}-
                                                        {{$motorcycle->state->name}}-
                                                        {{$motorcycle->city->name}}
                                                    </p>
                                                    <div class="ads_price_date d-flex justify-content-between">
                                                        <span class="price">
                                                            {{$motorcycle->price}} {{ $motorcycle->currency == 'Dollar' ? '$' : ($motorcycle->currency == 'Euro' ? '€' : 'FCFA') }}
                                                        </span>
                                                        <span class="date">{{$motorcycle->created_at->diffForHumans()}} </span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                @forelse ($motorcycles as $motorcycle)
                                    <div class="single_ads_card ads_list d-sm-flex mt-30">
                                        ...
                                        <div class="ads_card_content media-body">
                                            ...
                                            <h4 class="title">
                                                <a href="#">{{$motorcycle->category->category_name}}</a>
                                            </h4>
                                            <p><i class="far fa-map-marker-alt"></i>
                                                {{$motorcycle->country->name}}-
                                                {{$motorcycle->state->name}}-
